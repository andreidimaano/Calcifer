    ...
    case "pomodoro": {
      ...
      if (validPomodoro) {
        !args.workTime
          ...
          : await Pomodoro(message, args.workTime, args.breakTime);
      }
      break;
    }
    case "alltime": {
...
let canStartGroup = async (message: Message) => {
  ...
  if (groupPomInProgress) {
    ...
    await message.reply(
      "You are not connected to a group pomodoro voice channel"
    );
    return false;
  } else if (
...
let canHandleMessage = (message: Message): boolean => {
  return (
    !message.author.bot && message.content.toLowerCase().startsWith(prefix)
  );
};
